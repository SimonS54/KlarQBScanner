...
class OCRWorker(QtCore.QThread):
    ...
    def run(self):
        ...
        while self.running:
            try:
                ...
                error = self.extract_info(text, "Error =", validate_func=self.validate_error)
                qb_link = self.extract_info(text, "QB =", validate_func=self.validate_qb_link)
                qb_id = self.extract_info(text, "QB ID =", validate_func=self.validate_qb_id)
    ...
    def extract_info(self, text, label, validate_func=None):
        lines = text.split('\n')
        for line in lines:
            if label in line:
                extracted_info = line.split(label)[1].strip()
                if validate_func and not validate_func(extracted_info):
                    continue
                return extracted_info
        return None

    def validate_error(self, text):
        return len(text) > 0

    def validate_qb_link(self, text):
        return text.startswith("https://")

    def validate_qb_id(self, text):
        return len(text) == 8
